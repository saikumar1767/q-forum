...
export default function TodoList(props) {
  ...
  const addItem = () => {
    ...
    fetch("https://localhost:9999/question", {
      method: "POST",
  ...
  const deleteHandler = (itemIdx) => {
    ...
    fetch(`https://localhost:9999/question/${idToDelete}`, {
  ...
  const editHandler = (editedValue, itemIdx) => {
    ...
    fetch(`https://localhost:9999/question/${idToEdit}`, {
      method: "PUT",
  ...
  useEffect(() => {
    fetch("https://localhost:9999/questions", { credentials: "include"  })
      .then((r) => r.json())
      .then((arr) => {
        ...
        // const taskArr = sortedArr.map((item) => item.task); // gets the task for each item to create a string array
